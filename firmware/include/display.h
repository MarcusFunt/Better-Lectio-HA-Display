 ...
 * @param data_size image buffer size in bytes
 * @param bWait wait for the panel refresh to finish
 * @param bSkipClear skip the clearing pass before drawing
 * @return true when the display refresh succeeds
 */

bool display_show_image(uint8_t *image_buffer, int data_size, bool bWait, bool bSkipClear = false);

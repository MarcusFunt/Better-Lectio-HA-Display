 ...
 * @return none
 */
bool display_show_image(uint8_t *image_buffer, int data_size, bool bWait, bool bSkipClear)
...
void display_show_msg(uint8_t *image_buffer, MSG message_type, const char *message_text)
{
    ...
    break;
    case LECTIO_PROVISIONING_REQUIRED:
    {
        const char string1[] = "USB provisioning required.";
        bbep.getStringBox(string1, &rect);
        bbep.setCursor((bbep.width() - rect.w) / 2, 340);
        bbep.println(string1);
        const char string2[] = "Connect USB to configure WiFi and the display service.";
        bbep.getStringBox(string2, &rect);
        bbep.setCursor((bbep.width() - rect.w) / 2, -1);
        bbep.print(string2);
    }

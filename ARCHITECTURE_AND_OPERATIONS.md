...
The gateway admin and interactive login service remains on the Compose host's loopback-bound port `8000`. A separate `lectio-ha-api` service provides Home Assistant with bearer-token access to only the normalized, read-only `GET /api/v1/*` endpoints on host port `8002`. It binds to loopback by default. When Home Assistant runs on a separate LAN host, bind port `8002` to the Compose host's private LAN address and restrict the host firewall to the Home Assistant host; do not expose it to the public Internet.
...
- `/auth/browser` is the unified setup surface for display-to-Home-Assistant settings and preparation of the scoped API token used by HA.
- The Home Assistant long-lived token is stored only in a dedicated display-config volume, mounted read-only by the display service; it is not stored with Lectio sessions or display bitmaps.
- Explicit display disconnect removes the saved token and writes a secret-free managed marker, so legacy environment fallback cannot silently restore the connection.
- The scoped API token is generated by the gateway and only its SHA-256 digest is persisted in a separate volume, mounted read-only by the API proxy. Once a managed digest exists it overrides the legacy environment token.
- HACS installation, Home Assistant config-entry management, and private-calendar creation remain in the Home Assistant UI; the gateway does not call Home Assistant administrator APIs.
- Scoped API token rotation is completed through the existing Better Lectio config entry's **Reconfigure** action, which validates the replacement credential before updating and reloading that entry.
- Home Assistant URL/entity settings on the page take precedence over legacy environment settings once saved. Host port binding and firewall rules remain host-level configuration; the API stays loopback-bound by default.
